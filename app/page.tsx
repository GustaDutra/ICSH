...
export default function SpellListPage() {
  ...
  const [misterioFilter, setMisterioFilter] = useState<string>("Filtrar Mistério")

  const misterios = useMemo(() => {
    const set = new Set(SPELLS.flatMap((s) => s.misterio))
    return ["Filtrar Mistério", ...Array.from(set).sort()]
  }, [])

  /* Which mysteries have any spell matching current search/filter */
  const activeMisterios = useMemo(() => {
    if (!search && misterioFilter === "Filtrar Mistério") return new Set(MISTERIOS)
    return new Set(
      SPELLS
        .filter((s) => {
          ...
          const matchMisterio = misterioFilter === "Filtrar Mistério" || s.misterio.includes(misterioFilter)
    ...
    >
      ...
      <header style={{
        ...
        <Select value={misterioFilter} onValueChange={setMisterioFilter}>
            <SelectTrigger className="w-[160px] rounded-full border-border bg-muted text-foreground text-[0.8125rem] font-[inherit] cursor-pointer whitespace-nowrap">
              <SelectValue placeholder="Mistério" />
            </SelectTrigger>
            <SelectContent>
              {misterios.map((s) => (
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.75rem" }}>
          ...
          {search || misterioFilter !== "Filtrar Mistério" ? (
            <button
              onClick={() => { setSearch(""); setMisterioFilter("Filtrar Mistério") }}
              style={{
        ...
        <Accordion type="multiple">
          {MISTERIOS.map((misterio) => {
            ...
            const spellCount = SPELLS.filter((s) =>
              ...
              (misterioFilter === "Filtrar Mistério" || s.misterio.includes(misterioFilter)) &&
